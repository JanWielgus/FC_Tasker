...
void FC_SimpleTasker::addFunction(SimpleFunctionObject functionObject, long interv, uint16_t maxDur )
{
	...
	taskList[amtOfTasks-1].functionObject = functionObject;
...
void FC_SimpleTasker::runTasker()
{
	for (uint8_t i=0; i<amtOfTasks; i++)
	{
		...
		if (curTime > (taskList[i].lastExecuteTime + taskList[i].interval + taskList[i].timeShift))
		{
			...
			taskList[i].functionObject.functionCall_concept();
		}
	}
}
